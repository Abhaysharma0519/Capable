...
import AuditorOverviewTab from "../components/AuditorOverviewTab";
import AuditorClausesTab from "../components/AuditorClausesTab";
import AuditorControlsTab from "../components/AuditorControlsTab";
...
export default function AuditorDetails() {
  ...
  const [userName] = useState(
    localStorage.getItem("user_name") || "Auditor Name"
  );
  ...
  const [notifications, setNotifications] = useState([
    {
      id: 1,
      title: "Critical",
      desc: "Evidence pending for Control A.5",
      time: "1h ago",
      type: "urgent",
    },
    {
      id: 2,
      title: "Update",
      desc: "Clause 4.1 review completed",
      time: "3h ago",
      type: "info",
    },
  ]);
    ...
    <div className="flex min-h-screen w-full bg-[#F3F4F9] font-sans text-[#1F2937]">
      ...
      <main className="flex-1 px-4 py-24 md:px-10 md:py-8 lg:py-10 max-w-7xl mx-auto overflow-x-hidden">
        ...
        <header className="flex justify-between items-center mb-6">
          <nav className="flex items-center gap-2 text-xs md:text-xl font-medium text-gray-400">
            ...
            <span className="text-[#111827] font-bold">
              {id?.toUpperCase() || "ISO/IEC 27001:2022"}
            </span>
          </nav>
            ...
            <div className="relative">
              <div
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm border transition-all cursor-pointer 
                  ${
                    showNotifications
                      ? "bg-white border-[#6A5AFF] text-[#6A5AFF]"
                      : "bg-white border-gray-100 text-gray-400 hover:text-[#6A5AFF]"
                  }`}
              >
                <Bell size={18} />
              </div>
        ...
        <div className="mb-8 text-left">
          ...
          <div className="flex items-center gap-3">
            <span className="bg-indigo-50 text-[#6A5AFF] text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider">
              Information Security
            </span>
            <p className="text-sm text-slate-500 font-medium italic opacity-80">
              Viewing report for ClientName Pvt. Ltd.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 md:gap-8 border-b border-slate-200 mb-8 overflow-x-auto whitespace-nowrap">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 pb-4 px-1 text-sm font-bold transition-all relative ${
                activeTab === tab.name
                  ? "text-[#6A5AFF]"
        ...
        <div className="min-h-125">
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "Overview" && <AuditorOverviewTab />}
              {activeTab === "Clauses" && <AuditorClausesTab />}
              {activeTab === "Controls" && <AuditorControlsTab />}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
